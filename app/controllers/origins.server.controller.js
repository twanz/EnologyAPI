...
//	Origin.find().sort('-created').populate('user', 'displayName').exec(function(err, origins) {
	Origin.find().sort({name:1}).populate('user', 'displayName').exec(function(err, origins) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
...
exports.originByID = function(req, res, next, id) { 
	Origin.findById(id).populate('regions user').exec(function(err, origin) {
